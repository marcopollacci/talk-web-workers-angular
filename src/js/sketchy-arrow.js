...
const PROPS = Object.keys(DEFAULT_OPTIONS).map(key => `--sketchy-arrow-${key.replace(/[A-Z]/g, c => '-' + c.toLowerCase())}`);
...
registerPaint(
  ...
  class {
    ...
    drawArrow(ctx, geometry) {
      ...
      const [endX, endY] = this.approxPoint(geometry.width - this.variance, geometry.height - this.variance);
      ...
      ctx.arc(startX, rh + cosine + startY, rh + cosine, -Math.PI / 2, endAngle - Math.PI / 2);
